function toReadable(number) {
    ...
    if (99 < number && number < 1000) {
        result = result + numbers[0][number.toString().slice(-3, -2) - 1] + ' hundred '
    }
    if (19 < number.toString().slice(-2) && number.toString().slice(-2) < 100) {
        result = result + numbers[1][number.toString().slice(-2, -1) - 2] + ' '
        result = result + numbers[0][number.toString().slice(-1) - 1]
    }
    if (number.toString().slice(-2) < 19) result = result + numbers[0][number.toString().slice(-2) - 1]
